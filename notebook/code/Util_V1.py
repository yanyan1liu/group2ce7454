...
class DecayByEpoch(keras.callbacks.Callback):
    def on_epoch_end(self, epoch, log=[]):
        ...
        if initial_lr / new_lr > lr_minimum_rate_times:
...
# beside, if the learning rate has been reduced to a minimum  times, this process stops
class DecayByBatch(keras.callbacks.Callback):
    def on_batch_end(self, batch, log=[]):
        ...
        if initial_lr / new_lr > lr_minimum_rate_times:
            K.set_value(self.model.optimizer.lr, initial_lr/lr_minimum_rate_times)
        else:
            K.set_value(self.model.optimizer.lr, new_lr)
    def on_epoch_end(self, epoch, log=[]):
        lr = self.model.optimizer.lr
